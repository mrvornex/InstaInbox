...
import { createAccount, getMessages, getMessage, type MailTmSession } from "@/lib/mailtm";
...
export default function InboxCard() {
  ...
  const [availDomains, setAvailDomains]   = useState<string[]>([]);
  const [selDomain, setSelDomain]         = useState("");
  const [customUser, setCustomUser]       = useState("");
  const [creating, setCreating]           = useState(false);

  const sessionRef = useRef<MailTmSession | null>(null);
  ...
  const startPolling = useCallback(() => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      try {
        const msgs = await getMessages(sessionRef.current.token);
        setEmails((prev) => {
  ...
  // ── init ─────────────────────────────────────────────
  const initSession = useCallback(async (user?: string, domain?: string) => {
    setLoading(true);
    setEmails([]);
    try {
      const sess = await createAccount(user, domain);
      sessionRef.current = sess;
      setSession(sess);
      setCustomUser(sess.address.split("@")[0]);
      const exp = new Date(Date.now() + 60 * 60 * 1000);
      startTimer(exp);
      const msgs = await getMessages(sess.token);
      setEmails(msgs);
      startPolling();
    } catch (e) {
      console.error(e);
      showToast("⚠️ Could not create inbox. Retrying...");
      setTimeout(() => initSession(user, domain), 3000);
    } finally {
      setLoading(false);
    }
  }, [startTimer, startPolling]);

  // ── on mount: load domains then init ─────────────────
  useEffect(() => {
    (async () => {
      try {
        const res  = await fetch("/api/mailtm?action=domains");
        const data = await res.json();
        const doms: string[] = (data["hydra:member"] ?? []).map(
          (d: { domain: string }) => d.domain
        );
        setAvailDomains(doms);
        setSelDomain(doms[0] ?? "");
        await initSession(undefined, doms[0]);
      } catch {
        await initSession();
      }
    })();
  ...
  const handleRefresh = async () => {
    ...
    try {
      const msgs = await getMessages(sessionRef.current.token);
      setEmails(msgs);
      showToast(msgs.length > 0 ? `${msgs.length} email${msgs.length !== 1 ? "s" : ""} in inbox` : "Inbox is empty ✓");
    } catch {
      showToast("⚠️ Refresh failed.");
    } finally {
      setRefreshing(false);
    }
  };
  ...
  const handleNewAddress = async () => {
    if (pollRef.current)  clearInterval(pollRef.current);
    if (timerRef.current) clearInterval(timerRef.current);
    await initSession(undefined, selDomain || undefined);
  ...
  const handleCreateCustom = async () => {
    ...
    if (timerRef.current) clearInterval(timerRef.current);
    try {
      await initSession(customUser.trim(), selDomain || undefined);
      showToast("✓ Custom address created!");
    } catch {
      showToast("⚠️ Username taken or invalid. Try another.");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleOpenEmail = async (email: Email) => {
    ...
    if (sessionRef.current && email.body.length < 200) {
      try {
        const full = await getMessage(sessionRef.current.token, email.id);
        setSelectedEmail((prev) =>
          prev?.id === email.id ? { ...prev, body: full } : prev
        );
      } catch {
        // keep intro
      }
  ...
  const emailAddr   = session?.address ?? "Generating...";
    ...
    <>
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 bg-slate-50 border-b border-slate-200">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
              {loading ? "Creating Inbox..." : "Live Inbox — Real Emails"}
            </span>
          </div>
          {!loading && (
        ...
        <div className="px-5 pt-5 pb-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              <span className="font-mono text-sm text-slate-800 flex-1 truncate">
                {loading ? "Creating address..." : emailAddr}
              </span>
            ...
            {availDomains.length > 1 && (
              <select
                ...
                disabled={loading}
                className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm text-slate-700 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
              >
                {availDomains.map((d) => (
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <input
              type="text"
              value={customUser}
              onChange={(e) => setCustomUser(e.target.value.toLowerCase().replace(/[^a-z0-9._-]/g, ""))}
          ...
          {!loading && (
            <p className="mt-2 text-xs text-slate-400">
              💡 <strong>Real inbox</strong> — emails to{" "}
              <span className="font-mono text-blue-500 break-all">{emailAddr}</span>{" "}
        ...
        <div className="px-5 pb-5 flex flex-wrap gap-2">
          ...
          <button onClick={handleNewAddress} disabled={loading}
        ...
        <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 flex items-center gap-3">
          <span className="text-xs text-slate-500 flex-shrink-0">Expires in</span>
        ...
        <div className="border-t border-slate-200">
          ...
          {loading ? (
            <div className="text-center py-12 px-5">
              ...
              <p className="text-sm text-slate-500">Creating your inbox...</p>
            ...
            <div className="text-center py-12 px-5">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Send to <span className="font-mono text-blue-500">{emailAddr}</span>
              </p>
              <p className="text-xs text-slate-400 mt-0.5">Auto-checks every 10 seconds</p>
            </div>
          ) : (
            emails.map((email) => (
